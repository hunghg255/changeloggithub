...
export function parseCommits(commits: RawGitCommit[], config: ChangelogConfig): GitCommit[] {
  return commits.map(commit => parseGitCommit(commit, config)).filter(Boolean) as any
}
...
export function parseGitCommit(commit: RawGitCommit, config: ChangelogConfig): GitCommit | null {
  ...
  for (const v of commit.body.matchAll(CoAuthoredByRegex)) {
    if (v?.groups) {
      authors.push({
        name: (v.groups.name || '').trim(),
        email: (v.groups.email || '').trim(),
      })
    }
  }
